...
pub(crate) fn run(step: Step, input: Vec<String>) -> CustomResult<String> {
    let database: Vec<(Policy, String)> = input
        ...
        .map(|e| {
            ...
            let policy_char = &policy_parts[1].chars().nth(0).expect("raw policy to have exactly 1 char");

            let policy = Policy { min: range_start, max: range_end, letter: policy_char.to_owned() };
